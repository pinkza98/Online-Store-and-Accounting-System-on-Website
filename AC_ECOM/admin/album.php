                ...
                <div class="container-fluid">
                        ...
                        <div class="col-lg-12 p-r-0 title-margin-right">
                            <div class="page-header">
                                <div class="page-title">
                                    <h1>หน้า, <span>รายการเก็บรูปภาพบิล-ใบเสร็จซื้อของ</span></h1>
                                </div>
                            </div>
                        </div>
                    </div>
                    <!-- /# row -->
                     <section id="main-content" >
                        <div class="row" >
                            <div class="col-lg-6" >
        <form action="album_add_db.php" method="post" name="product" enctype="multipart/form-data" >
                <table class='table table-bordered'>
                                    <select class="form-control input-default" name="algroup_id" id="categories">
                                      <option selected disabled>----โปรดเลือกชื่ออัลบั้ม----</option>
                                      <?php
                                      $qr1 = mysqli_query($mysqli, "SELECT * FROM tb_album_group ORDER BY algroup_id");

                                      while($dat1 = mysqli_fetch_assoc($qr1)){
                                          ?>
                                          <option value="<?=$dat1['algroup_id'];?>"><?=$dat1['algroup_name'];?></option>
                                          <?php
                                      }
                                      ?>
                                    </select>
                            ...
                            <tr>
                                <br>
                            <input class="form-control input-default" type="file" name="album_image[]" multiple="">
                            <br>
                         ...
                         <div class="col-lg-6">
                     <form action="album_group_add_db.php" method="post" name="product" enctype="multipart/form-data">

                                    <input class="form-control input-default " type="text" placeholder="ชื่ออัลบั้ม" name="algroup_name"><br>
                        ...
                        <!-- /# row -->
                            ...
                            <!-- column -->
                            <?php
                            $qr_group = mysqli_query($mysqli, "SELECT * FROM tb_album_group");
                            while($data_group = mysqli_fetch_assoc($qr_group)){
                              ?>
                              <div class="row">
                                <div class="col-lg-12">
                                  <?php
                                    echo "<h3>".$data_group['algroup_name']."</h3>";
                                  ?>
                                </div>
                                <?php
                                $algroup_id = $data_group['algroup_id'];
                                $qr_album = mysqli_query($mysqli, "SELECT * FROM tb_album WHERE algroup_id='$algroup_id' ORDER BY album_id DESC");
                                while($data_album = mysqli_fetch_assoc($qr_album)){
                                  ?>
                                  <div class="col-lg-2">
                                    <img src="img_album/<?=$data_album['album_image'];?>" width="100%">
                                  </div>
                                  <?php
                                }
                                ?>
